...
boolean Pushover::send(void) {
	WiFiClientSecure clientPush;
	clientPush.setInsecure();
	if (!clientPush.connect("api.pushover.net", 443)) {
		return false;
		//Serial.println("couldnot connect");
	}
	...
	clientPush.print(http);
	int timeout_at = millis() + _timeout;
	while (!clientPush.available() && timeout_at - millis() < 0) {
		clientPush.stop();
		return false;
	}
	String line;
	while (clientPush.available() != 0) {
		if (clientPush.read() == '{') break;
	}
	line = clientPush.readStringUntil('\n');
